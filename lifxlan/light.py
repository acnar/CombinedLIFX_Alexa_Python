...
class Light(Device):
    ...
    def set_power(self, power, duration=0, rapid=False):
        ...
        off = [False, 0, "off"]
        try:
            #print("set power duration = %i", duration)
    ...
    def get_color(self):
        try:
            ...
            self.power_level = response.power_level
            self.label = response.label.decode().strip("\0\t\n")
        except WorkflowException as e:
            raise
        return self.color
    ...
    def set_hsbk(self, color, duration=0, rapid=False):
        #print("setting %s with duration %i" % (self.label, duration))
        try:
            if rapid:
                self.fire_and_forget(LightSetColor, {"color": color, "duration": duration}, num_repeats=5)
            else:
            ...
            raise
        #print("done setting %s" % self.label)
        return True
