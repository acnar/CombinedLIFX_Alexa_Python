...
class MediaLibrary:
    ...
    def __init__(self):
        ...
        for key in self.config.keys():
            if "path" in key:
                self.directories.append(self.config[key])
    ...
    def list_seasons(self, show_name):
        ...
        try:
            ...
            seasons = [x for x in seasons if mediautlils.seasonnumber(x) is not None]
            return seasons
        except Exception as e:
    ...
    def find_episode(self, showname, season, episodenum):
        ...
        if episodes is None:
            ...
            return None
        candidates = [x for x in episodes if mediautlils.episodenumber(x) == episodenum]
        if len(candidates) > 0:
            return candidates[0]
        return None
